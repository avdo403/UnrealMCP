...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("toggle_flashlight"))
    {
        return HandleToggleFlashlight(Params);
    }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // If still not found, try common engine paths for components
    if (!ComponentClass)
    {
        TArray<FString> CommonPaths;
        CommonPaths.Add(FString::Printf(TEXT("/Script/Engine.%s"), *ComponentType));
        CommonPaths.Add(FString::Printf(TEXT("/Script/Engine.U%s"), *ComponentType));
        
        FString CleanType = ComponentType;
        if (CleanType.StartsWith(TEXT("U"))) CleanType = CleanType.RightChop(1);
        if (!CleanType.EndsWith(TEXT("Component"))) CleanType += TEXT("Component");
        
        CommonPaths.Add(FString::Printf(TEXT("/Script/Engine.%s"), *CleanType));
        CommonPaths.Add(FString::Printf(TEXT("/Script/Engine.U%s"), *CleanType));

        for (const FString& Path : CommonPaths)
        {
            ComponentClass = LoadClass<UActorComponent>(nullptr, *Path);
    ...
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown component type: %s. Try using precise name like 'SpotLightComponent'"), *ComponentType));
    }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleSetComponentProperty(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // 1. Check SimpleConstructionScript (Components added in Blueprint)
    ...
    // 2. If not found, check Inherited Components (Components from C++ parent class)
    if (!TargetObject)
    {
        UClass* TargetClass = Blueprint->GeneratedClass;
        if (!TargetClass) TargetClass = Blueprint->ParentClass;
        ...
        if (TargetClass)
        {
            ...
            for (TFieldIterator<FProperty> It(TargetClass); It; ++It)
            {
                ...
                if (Prop->GetName() == ComponentName)
                {
                    ...
                    {
                        TargetObject = ObjProp->GetObjectPropertyValue(ObjProp->ContainerPtrToValuePtr<void>(CDO));
                        break;
                    }
                }
            }
    ...
    if (!TargetObject)
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Component '%s' not found in blueprint '%s'"), *ComponentName, *BlueprintName));
    }

    FString ErrorMessage;
    if (FEpicUnrealMCPCommonUtils::SetObjectProperty(TargetObject, PropertyName, PropertyValue, ErrorMessage))
    {
        // Mark blueprint as dirty and potentially compile
        Blueprint->MarkPackageDirty();
        FKismetEditorUtilities::CompileBlueprint(Blueprint);
        
        TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
        ResultObj->SetBoolField(TEXT("success"), true);
        ResultObj->SetStringField(TEXT("message"), FString::Printf(TEXT("Set property '%s' on component '%s'"), *PropertyName, *ComponentName));
        return ResultObj;
    }

    return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to set property: %s"), *ErrorMessage));
}
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleReadBlueprintContent(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (bIncludeComponents)
    {
        ...
        // 1. SCS Components (Added directly in this Blueprint)
        ...
        // 2. Inherited Components (Defined in C++ or parent Blueprints)
        // We look at the GeneratedClass or ParentClass to find component properties
        ...
        if (SearchClass)
        {
            for (TFieldIterator<FObjectProperty> It(SearchClass); It; ++It)
            {
                ...
                {
                    // Check if we already added this from SCS (SCS components often have properties too)
                    FString CompName = ObjProp->GetName();
                    bool bAlreadyAdded = false;
                    for (const TSharedPtr<FJsonValue>& Existing : ComponentArray)
                    {
                        if (Existing->AsObject()->GetStringField(TEXT("name")) == CompName)
                        {
                            bAlreadyAdded = true;
                            break;
                        }
                    ...
                    if (!bAlreadyAdded)
                    {
                        ...
                        CompObj->SetStringField(TEXT("class"), ObjProp->PropertyClass->GetName());
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleToggleFlashlight(const TSharedPtr<FJsonObject>& Params)
{
    FString ActorName;
    ...
    {
        TargetActor = FEpicUnrealMCPCommonUtils::FindActorByName(ActorName);
    }
    else
    {
        // Try to find the local player's pawn if no actor specified
        if (GEditor && GEditor->GetEditorWorldContext().World())
        {
            if (APlayerController* PC = GEditor->GetEditorWorldContext().World()->GetFirstPlayerController())
            {
    ...
    if (!TargetActor)
    {
        // Fallback: look for BP_ThirdPersonCharacter specifically if it exists
        TargetActor = FEpicUnrealMCPCommonUtils::FindActorByName(TEXT("BP_ThirdPersonCharacter"));
    }

    if (!TargetActor)
    {
        return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Could not find target actor for flashlight"));
    }

    // Check if it already has a spotlight component named "MCP_Flashlight"
    USpotLightComponent* Flashlight = nullptr;
    TArray<USpotLightComponent*> Components;
    TargetActor->GetComponents<USpotLightComponent>(Components);
    
    for (USpotLightComponent* Comp : Components)
    {
        if (Comp->GetName().Contains(TEXT("MCP_Flashlight")))
        {
            Flashlight = Comp;
            break;
        }
    ...
    if (Flashlight)
    {
        ...
        Result->SetBoolField(TEXT("is_on"), bNewVisibility);
        Result->SetStringField(TEXT("status"), TEXT("toggled"));
        return Result;
    }
    else
    {
        // Add a new spotlight component
        Flashlight = NewObject<USpotLightComponent>(TargetActor, TEXT("MCP_Flashlight"));
        if (Flashlight)
        {
            Flashlight->RegisterComponent();
            Flashlight->AttachToComponent(TargetActor->GetRootComponent(), FAttachmentTransformRules::KeepRelativeTransform);
            
            // Configure flashlight properties
            Flashlight->SetRelativeLocation(FVector(0, 0, 100)); // Slightly above center
            ...
            TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
            Result->SetStringField(TEXT("actor"), TargetActor->GetName());
            Result->SetBoolField(TEXT("is_on"), true);
            Result->SetStringField(TEXT("status"), TEXT("created"));
            return Result;
        }
    }

    return FEpicUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Failed to create flashlight component"));
}