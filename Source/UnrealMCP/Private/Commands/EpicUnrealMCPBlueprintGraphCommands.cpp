...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintGraphCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    else if (CommandType == TEXT("import_nodes_from_t3d"))
    {
        return HandleImportNodesFromT3D(Params);
    }

    return FEpicUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown blueprint graph command: %s"), *CommandType));
}

TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintGraphCommands::HandleImportNodesFromT3D(const TSharedPtr<FJsonObject>& Params)
{
    return FBlueprintNodeManager::ImportNodesFromT3D(Params);
}
