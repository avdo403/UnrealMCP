...
void FEpicUnrealMCPCommonUtils::AddVectorToJson(TSharedPtr<FJsonObject> JsonObject, const FString& FieldName, const FVector& Vector)
{
...
void FEpicUnrealMCPCommonUtils::AddRotatorToJson(TSharedPtr<FJsonObject> JsonObject, const FString& FieldName, const FRotator& Rotator)
{
    if (!JsonObject.IsValid()) return;
...
UBlueprint* FEpicUnrealMCPCommonUtils::FindBlueprintByName(const FString& BlueprintName)
{
    ...
    if (BlueprintName.StartsWith(TEXT("/")))
    {
        ...
        {
            // It's already a fully qualified object path (e.g., /Game/Path.Asset)
            ObjectPath = BlueprintName;
        }
        else
        {
            // It's a package path, append the asset name (e.g., /Game/Path -> /Game/Path.Path)
    ...
    if (!Blueprint)
    {
         // If path-based search failed, try searching for the blueprint by name globally in the asset registry
         UE_LOG(LogTemp, Display, TEXT("FindBlueprintByName: Path-based lookup failed for '%s', searching globally by name"), *BlueprintName);
         ...
         {
             if (Data.AssetName.ToString().Equals(BlueprintName, ESearchCase::IgnoreCase))
             {
                 Blueprint = Cast<UBlueprint>(Data.GetAsset());
                 if (Blueprint)
                 {
                     UE_LOG(LogTemp, Display, TEXT("FindBlueprintByName: Found blueprint '%s' at path '%s' via global name search"), 
                         *BlueprintName, *Data.GetObjectPathString());
                     return Blueprint;
                 }
             }
         }
         
         // Last ditch - load by object name if it's already in memory
         Blueprint = FindObject<UBlueprint>(nullptr, *BlueprintName);
    }
...
// Actor utilities
AActor* FEpicUnrealMCPCommonUtils::FindActorByName(const FString& ActorName)
{
    ...
    for (TObjectIterator<AActor> It; It; ++It)
    {
        ...
        if (Actor && !Actor->IsTemplate() && 
            (Actor->GetName() == ActorName || Actor->GetActorLabel() == ActorName))
        {
            return Actor;
        }
    }
    ...
    else if (Property->IsA<FStructProperty>())
    {
        ...
        if (StructProp->Struct == TBaseStructure<FVector>::Get())
        {
            if (Value->Type == EJson::Array)
            {
                ...
                if (Array.Num() >= 3)
                {
                    FVector Vector(Array[0]->AsNumber(), Array[1]->AsNumber(), Array[2]->AsNumber());
                    StructProp->CopyCompleteValue(PropertyAddr, &Vector);
                    return true;
                }
            }
        }
        else if (StructProp->Struct == TBaseStructure<FRotator>::Get())
        {
            if (Value->Type == EJson::Array)
            {
                ...
                if (Array.Num() >= 3)
                {
                    FRotator Rotator(Array[0]->AsNumber(), Array[1]->AsNumber(), Array[2]->AsNumber());
                    StructProp->CopyCompleteValue(PropertyAddr, &Rotator);
                    return true;
                }
            }
        }
        else if (StructProp->Struct == TBaseStructure<FLinearColor>::Get())
        {
            if (Value->Type == EJson::Array)
            {
                ...
                if (Array.Num() >= 3)
                {
                    FLinearColor Color(Array[0]->AsNumber(), Array[1]->AsNumber(), Array[2]->AsNumber(), 
                                      Array.Num() >= 4 ? Array[3]->AsNumber() : 1.0f);
